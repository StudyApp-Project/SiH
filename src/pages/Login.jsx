...
export default function Login() {
  ...
  const { login, loginWithGoogle, loginWithGithub, error: authError, clearError } = useUser();
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // onAuthStateChanged in UserContext handles the redirect trigger,
      // but the ProtectedRoute guard will auto-redirect to dashboard
      navigate('/dashboard');
    } catch (err) {
    ...
    >
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <button 
          ...
          className="relative overflow-hidden group flex items-center justify-center gap-2 w-full py-2.5 px-4 rounded-xl border border-(--border-default) bg-(--bg-glass) hover:border-(--border-strong) transition-colors font-medium text-sm"
        >
