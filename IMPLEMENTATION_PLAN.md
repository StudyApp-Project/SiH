...
- **Ground Field Personnel**: Field Investigator / Primary Enumerator (NSSO Field Operations Division - FOD) conducting PLFS, ASI, ASUSE, and agricultural surveys on CAPI tablets.
...
| Multilingual (Hindi/English)       | i18n framework + translation layer                    |
...
| Mobile PWA / Offline Support       | Service worker + IndexedDB sync                       |
...
- [ ] **1.3 Domain Data Framework (FRAC-Aligned & MoSPI Grounded)**:
  ...
  - [ ] Create `src/data/translations.js`:
    - Lightweight English/Hindi i18n dictionary for navigation items, competency names, and key statistical terms.
...
- Language toggle switches header navigation and role labels between English and Hindi.
...
The end-to-end demo should be a single, coherent story that demonstrates the closed loop:

### Scene 1: Learner Onboarding (2 min)

1. Sign in via one-click **Parichay (Simulated Jan-Parichay Gov SSO)** as **Junior Statistical Officer (JSO) Amit Sharma**
2. Details pre-populated: NSSO (Field Operations Division), Ministry of MoSPI
3. System loads required FRAC competencies for JSO: Survey Sampling, CAPI Data Collection, National Accounts, Data Governance
4. Learner completes self-assessment: marks **Survey Sampling & Field Estimation** at Level 1 (✍️ self-assessed)
5. **First impression**: Learner Dashboard displays Workforce Readiness Index at ~54%, Karma Points at 120, and APAR appraisal progress

### Scene 2: Discovering Priority-Weighted Gaps (1 min)

6. Navigate to **Skill Gap Analysis**
7. See priority-ranked gaps: **Survey Sampling (L1 → L3, Critical 🔴)** ranks top due to activity weighting ($\Delta=2 \times 3 = 6$)
8. AI Explain-the-Gap narrator shows: _"The Activity 'Conduct NSS Household Surveys' requires the Competency 'Survey Sampling & Estimation' at Level 3 per the JSO FRAC mapping — your current level is 1 (self-assessed)."_
9. Learner clicks **"Close This Gap"** on Survey Sampling

### Scene 3: Learning Recommendations & iGOT Deep-Link (1 min)

10. **Learning Pathways** page highlights AI-recommended courses:
    - _"Advanced Survey Sampling & Estimation (NSSTA-202)"_ — 40 Karma Points
    - Shows "Why Recommended" card: _"Directly addresses your Critical Gap for Activity 'Conduct NSS Household Surveys' — 40 Karma Points will boost your APAR readiness milestone."_
11. Learner clicks **"View on iGOT Karmayogi"** $\to$ deep-links to live iGOT platform catalog
12. Learner marks module completed $\to$ Karma Points increase to 160

### Scene 4: Trainer Ingests Manual & Generates Content (2 min)

13. **Switch to Trainer Account** (NSSTA Faculty Dr. Rao)
14. Upload official training manual: _"MoSPI Guidelines on Household Survey Sampling.pdf"_ (selects Chapters 1–3, 14 pages)
15. Configure: 12 questions, medium difficulty, competency: **Survey Sampling & Estimation**
16. AI generates 12 questions in a single batch request with confidence tags (`high`, `medium`, `low`)
17. **Stage 5a Validation**: Sanity check confirms topics match the Survey Sampling competency
18. Review Queue: Low-confidence questions appear first. Trainer approves 10, edits 1, rejects 1
19. Publish approved questions to the **Question Bank**

### Scene 5: Learner Takes Adaptive Assessment (2 min)

20. Return to **Learner Account** (Amit Sharma)
21. Start **Survey Sampling Competency Assessment** (dynamically pulled from the newly approved Question Bank)
22. Focus-mode interface with timer, question navigation, and **"Flag Question"** button
23. **Adaptive Item Branching in action**: Answering Question 1 correctly branches to a higher difficulty item; answering Question 3 incorrectly branches to an easier item to calibrate exact L1–L5 level
24. Toggle language to **हिंदी** to preview question stems in Hindi
25. Submit assessment

### Scene 6: The Loop Closes & Verification Updates (1 min)

26. Results screen displays: 82% overall score
    - Stratified Sampling: 90% ✅
    - Sample Weighting & Multipliers: 80% ✅
    - Non-Sampling Error Control: 65% ⚠️
27. Post-assessment AI micro-feedback explains: _"Great work on stratification formulas. You missed 1 question on multiplier calculation — Chapter 3 of the NSSTA manual addresses estimation formulas."_
28. **Competency impact**: Survey Sampling promotes L1 $\to$ L2 with a verified badge (🛡️ assessment-verified)
29. Profile updates: Readiness Index climbs to 65%; Skill Gap page reflects the closed gap

### Scene 7: Administrator Workforce Intelligence (1 min)

30. **Switch to Admin Account** (Director General, MoSPI)
31. Cadre Overview: 18 officers onboarded across NSSO FOD, CSO, and SDRD; average readiness 64%
32. AI Narrative Summary displays: _"Survey Sampling proficiency among JSOs in NSSO FOD has improved +11% following recent NSSTA assessments. Field Data Collection remains at Level 1 for 6 officers."_
33. Admin clicks **"Flag for Priority Training"** on Field Data Collection $\to$ demonstrates administrative write-back action
...
### Action Items (Not Blocking Development)

- [ ] **iGOT outreach**: Actually email `mission.karmayogi@gov.in` with a one-pager about the project. Cost: nothing. Value: real answer to "did you try?" instead of just an architectural mock.
